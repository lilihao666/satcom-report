...
def fix_file(path, is_premium=False):
    with open(path, 'r', encoding='utf-8') as f:
        html = f.read()

    # ── 1) 载荷卡片添加 clickable-card + onclick ──
    # 国内载荷（border-blue-500）
    domestic_payloads = [
        ...
        ('星移联信', 'IoT NTN/5G NR NTN', '国内首家专注卫星通信载荷系统设计的商业航天公司，2026年池州工厂投产，年产300台载荷', '星载基站|星载路由|卫星物联网载荷'),
    ...
    # Replace domestic payload divs
    for name, tech, desc, tags in domestic_payloads:
        old = f'<div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">\n                <h4 class="font-bold text-lg">{name}</h4>'
        key = f"payload_{name.replace(' ', '_')}"
    ...
    # Replace international payload divs
    for name, tech, desc, tags in intl_payloads:
...
'''
    # Find the international section header and insert before it
    intl_header = '<h3 class="text-lg font-semibold mb-3 text-gray-700 flex items-center"><span class="category-badge cat-internet mr-2">国际载荷厂商</span></h3>'
    html = html.replace(intl_header, yixin_card + intl_header, 1)

    # ── 4) 添加平天星座 ──
    # 平天星座是星移联信规划的星座，60+颗异构化卫星，定位于卫星互联网
    pingtian_modal = {
        "const_平天星座": {
            "title": "平天星座",
    ...
    pingtian_json = json.dumps(pingtian_modal, ensure_ascii=False, separators=(',', ':'))[1:-1]
    html = html.replace('"const_蓝凌星通":', f'{pingtian_json},"const_蓝凌星通":', 1)

    # Add 平天星座 card in constellation section - find the "国内卫星物联网星座" section
    # Actually we need to find the right place. Let me add it as a card in the constellation grid
    pingtian_card = '''<div class="bg-white rounded-lg shadow p-4 border-l-4 border-green-500 clickable-card relative" onclick="openModal('const_平天星座')">
...
'''
    # Find 蓝凌星通 card and insert before it
    lanling_marker = '<div class="bg-white rounded-lg shadow p-4 border-l-4 border-green-500 clickable-card relative" onclick="openModal(\'const_蓝凌星通\')">'
    html = html.replace(lanling_marker, pingtian_card + lanling_marker, 1)

    # ── 5) 添加更多最新动态 ──
    # Find the news section and add 5 more items
    new_news = '''
...
'''
    # Insert before the closing </div></section> of news
    # The news section ends with "<!-- 技术趋势 -->" or similar
    news_end_marker = '<!-- 技术趋势 -->'
    html = html.replace(news_end_marker, new_news + '            </div>\n        </section>\n        ' + news_end_marker, 1)
    # Wait - that might break structure. Let me be more careful.
    # Actually I need to just insert inside the news div before its closing
    # Let me replace more carefully - find the last news item before the tech trends section
    # The news section has: </div> </section> before 技术趋势
    # Let me look for the pattern: last news item is about 10th one
    # I'll replace the closing </div> of news container
    # Actually easier: find "<!-- 技术趋势 -->" and insert the news before it, but we need to close the news grid first
    # Let me revert and do it differently
    pass  # Will handle this more carefully below
...
# Actually let me do this step by step with more precise string operations
