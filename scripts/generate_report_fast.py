...
def generate_summary(data):
    """生成执行摘要"""
    total_terminals = len(data['companies']['domestic']) + len(data['companies']['international'])
    total_constellations = len(data['constellations']['domestic']) + len(data['constellations']['international'])
...
def generate_constellations(data):
    ...
    for const in data['constellations']['domestic']:
        if const.get('category') == '卫星互联网':
            html.append(generate_constellation_card(const))
    html.append('</div>')
...
def generate_constellation_card(const):
    ...
    if inter_satellite.get('enabled'):
        ...
        isl_html = f'''<div class="bg-blue-50 p-3 rounded-lg">
            <p class="font-semibold text-blue-800 mb-2"><i class="fas fa-satellite mr-1"></i> 星间链路</p>
            <p class="text-sm text-blue-700"><strong>状态:</strong> ✅ 支持</p>
            <p class="text-sm text-blue-700"><strong>技术:</strong> {inter_satellite.get('tech', '未知')}</p>
            {bandwidth_line}
            {note_line}
        </div>'''
    else:
        isl_html = f'''<div class="bg-gray-50 p-3 rounded-lg">
            <p class="font-semibold text-gray-700 mb-2"><i class="fas fa-satellite mr-1"></i> 星间链路</p>
            <p class="text-sm text-gray-600">{inter_satellite.get('note', '无星间链路')}</p>
        </div>'''
    ...
    modal_content = f'''<div class="space-y-4">
        <div class="flex justify-between items-center">
            ...
            <span class="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded">{const.get('stage', '规划中')}</span>
        </div>
        <div>
            <p class="text-gray-600"><strong>运营方:</strong> {const.get('operator', '未知')}</p>
            <p class="text-gray-600"><strong>规划数量:</strong> {const.get('planned', 0):,} 颗</p>
            <p class="text-gray-600"><strong>已发射:</strong> {const.get('launched', 0)} 颗</p>
        </div>
        {isl_html}
        <div>
    ...
    return f'''
        <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500 clickable-card relative" onclick="openModal('{const['name']}', '{escape_js_string(modal_content)}')">
...
def generate_terminals(data):
    ...
    if iot_companies:
        ...
        for company in iot_companies:
            html.append(generate_terminal_card(company))
        html.append('</div>')
    ...
    for company in data['companies']['international']:
        html.append(generate_terminal_card(company, is_international=True))
    html.append('</div>')
...
def generate_terminal_card(company, is_international=False):
    ...
    modal_content = f'''<div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <p class="font-medium">{company.get('founded', '未知')}</p>
            </div>
            <div>
                <p class="text-sm text-gray-500">融资阶段</p>
                <p class="font-medium"><span class="bg-blue-100 text-blue-800 px-2 py-0.5 rounded">{company.get('funding', '未知')}</span></p>
            </div>
            <div>
                <p class="text-sm text-gray-500">核心技术</p>
                <p class="font-medium">{company.get('tech', '未知')}</p>
            </div>
        </div>
        <div>
            <p class="text-sm text-gray-500 mb-2">主营业务</p>
            <p class="text-gray-700">{company.get('focus', '未知')}</p>
        </div>
        <div>
            <p class="text-sm text-gray-500 mb-2">公司简介</p>
            <p class="text-gray-700">{company.get('description', '暂无描述')}</p>
        </div>
        <div>
    ...
    return f'''
        <div class="bg-white rounded-lg shadow p-4 hover:shadow-lg transition clickable-card relative" onclick="openModal('{company['name']}', '{escape_js_string(modal_content)}')">
        ...
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">中国星网</span>
                    <span class="text-sm text-gray-500">GW星座运营</span>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">垣信卫星</span>
                    <span class="text-sm text-gray-500">千帆星座运营</span>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">国电高科</span>
                    <span class="text-sm text-gray-500">天启星座运营</span>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">时空道宇</span>
                    <span class="text-sm text-gray-500">吉利未来星座</span>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">中国卫通</span>
                    <span class="text-sm text-gray-500">中星系列</span>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">SpaceX Starlink</span>
                    <span class="text-sm text-gray-500">全球最大星座</span>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">Eutelsat OneWeb</span>
                    <span class="text-sm text-gray-500">企业宽带服务</span>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">Amazon Kuiper</span>
                    <span class="text-sm text-gray-500">部署中</span>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">Iridium</span>
                    <span class="text-sm text-gray-500">卫星电话/物联网</span>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 rounded">
                    <span class="font-medium">Intelsat/SES</span>
                    <span class="text-sm text-gray-500">GEO卫星运营</span>
                </div>
            </div>
        </div>
    </div></section>'''
...
def generate_news(data):
    ...
    for item in data.get('news', [])[:10]:  # 只显示前10条
...
def main():
    ...
    report = report.replace('{{constellations}}', generate_constellations(data))
    report = report.replace('{{terminals}}', generate_terminals(data))
